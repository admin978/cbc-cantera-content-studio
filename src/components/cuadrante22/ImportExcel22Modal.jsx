...
export default function ImportExcel22Modal({ open, onOpenChange, cuadranteId, onImported }) {
  ...
  const handleConfirm = async () => {
    ...
    setSaving(true);
    if (mode === "replace") {
      const existing = await base44.entities.PartidoCantera22.filter({ cuadrante: cuadranteId });
      await Promise.all(existing.map((p) => base44.entities.PartidoCantera22.delete(p.id)));
    }
    await base44.entities.PartidoCantera22.bulkCreate(
      preview.map((p) => ({
          ...
          <div className="space-y-4">
            ...
            <RadioGroup value={mode} onValueChange={setMode} className="flex gap-6">
              <div className="flex items-center gap-2">
                <RadioGroupItem value="replace" id="r22-1" />
                <Label htmlFor="r22-1" className="text-sm cursor-pointer">Reemplazar datos actuales</Label>
              </div>
              <div className="flex items-center gap-2">
                <RadioGroupItem value="add" id="r22-2" />
                <Label htmlFor="r22-2" className="text-sm cursor-pointer">Añadir a los existentes</Label>
              </div>
            </RadioGroup>
